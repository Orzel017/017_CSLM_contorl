...
# can ommit bc not immediately needed?
# this class requires "class DAQDigitalOutputState(Parameter)"
...
class DAQDigitalInputs(Instrument):
    ...
    def __init__(self, name: str, dev_name: str, rate: Union[int, float], channels: Dict[str, int],
        ...
        self.metadata.update({
            ...
            'channels': channels})
        for ch, idx in channels.items():
            channel = f'{dev_name}/di{idx}'
            self.task.di_channels.add_di_voltage_chan(channel, ch, min_val=min_val, max_val=max_val)
        if clock_src is None:
        ...
        self.add_parameter(
            name='voltage',
            parameter_class=DAQDigitalInputVoltages,
            task=self.task,
            samples_to_read=samples_to_read,
            shape=(nchannels, target_points),
            timeout=timeout,
...
class DAQDigitalInputVoltages(ArrayParameter):
